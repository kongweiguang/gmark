...
fn sidecar_cleanup_preserves_current_entry_and_enforces_budget() {
    ...
    let keep = dir.path().join("keep.gmark-json-v1");
    let stale_a = dir.path().join("a.gmark-json-v1");
    let stale_b = dir.path().join("b.gmark-json-v1");
    std::fs::write(&keep, [0u8; 10]).unwrap();
    std::fs::write(&stale_a, [0u8; 10]).unwrap();
    std::fs::write(&stale_b, [0u8; 10]).unwrap();

    cleanup_json_sidecars(dir.path(), &keep, 15).unwrap();
