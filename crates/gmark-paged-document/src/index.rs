...
impl Utf8StreamValidator {
    fn push(&mut self, bytes: &[u8]) -> Result<(), PagedDocumentError> {
        if memchr(0, bytes).is_some() {
            return Err(PagedDocumentError::Binary);
        }
        let mut start = 0usize;
        if self.trailing_len > 0 {
            let width = utf8_sequence_width(self.trailing[0]).ok_or(PagedDocumentError::Binary)?;
            ...
            std::str::from_utf8(&self.trailing[..self.trailing_len])
                .map_err(|_| PagedDocumentError::Binary)?;
            self.trailing_len = 0;
        }

        match std::str::from_utf8(&bytes[start..]) {
            Ok(_) => Ok(()),
            Err(error) if error.error_len().is_some() => Err(PagedDocumentError::Binary),
            Err(error) => {
    ...
    fn finish(self) -> Result<(), PagedDocumentError> {
        if self.trailing_len == 0 {
            Ok(())
        } else {
            Err(PagedDocumentError::Binary)
        }
    }
}
...
impl LinePages {
    ...
    fn compact(&self) -> Result<Self, PagedDocumentError> {
        let mut spool = PageSpool::new()?;
        for page_index in 0..self.len() {
            let page = self
                .page(page_index)
                .ok_or_else(|| PagedDocumentError::Io {
...
impl LineIndex {
    pub fn build(source: &FileSource) -> Result<Self, PagedDocumentError> {
    ...
    pub fn extend_for_append(&self, source: &FileSource) -> Result<Self, PagedDocumentError> {
    ...
    pub fn extend_for_append_cancellable(
        ...
        let mut builder = LineIndexBuilder::resume_append(self, new_len)
            .ok_or(PagedDocumentError::SourceChanged)?;
        let mut validator = Utf8StreamValidator::default();
    ...
    fn load_sidecar(source: &FileSource, path: &Path) -> Result<Option<Self>, PagedDocumentError> {
        ...
        let file = std::fs::File::open(path).map_err(|source| PagedDocumentError::Io {
    ...
    fn store_sidecar(&self, source: &FileSource, path: &Path) -> Result<(), PagedDocumentError> {
        ...
        std::fs::create_dir_all(parent).map_err(|source| PagedDocumentError::Io {
