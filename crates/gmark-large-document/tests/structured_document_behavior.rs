...
use gmark_large_document::{
    DelimitedFilterOptions, DelimitedIndex, DelimitedIndexOptions, FileSource, JsonIndex,
    JsonIndexOptions, JsonRootKind, LargeRecoveryJournal, LargeRecoveryReadStatus,
    LargeRecoverySelection, LineIndex, MarkdownTableIndex, PieceDocument, SearchCancellation,
    SourceAffinity, SourceAnchor, TextEncoding, replay_large_recovery,
...
fn json_index_rejects_invalid_syntax_with_a_source_byte_offset() {
    ...
    let gmark_large_document::LargeDocumentError::InvalidJson { offset, message } = error else {
...
#[test]
fn large_recovery_keeps_base_on_disk_and_replays_edits_undo_redo_and_truncated_tail() {
    ...
    let mut journal = LargeRecoveryJournal::create(
    ...
    let selection = Some(LargeRecoverySelection {
    ...
    let recovered = replay_large_recovery(&journal_path).unwrap();
    assert_eq!(recovered.read_status, LargeRecoveryReadStatus::Complete);
    ...
    fs::OpenOptions::new()
        ...
        .write_all(b"GMRJ")
        .unwrap();
    let recovered_tail = replay_large_recovery(&journal_path).unwrap();
    assert_eq!(
        recovered_tail.read_status,
        LargeRecoveryReadStatus::TruncatedTail
    );
    assert_eq!(
        recovered_tail
            .document
...
#[test]
fn large_recovery_refuses_a_changed_base_file() {
    let dir = tempfile::tempdir().unwrap();
    let source_path = dir.path().join("changed-base.txt");
    fs::write(&source_path, b"original").unwrap();
    let source = FileSource::open(&source_path).unwrap();
    let journal = LargeRecoveryJournal::create(
        dir.path().join("recovery"),
        &source,
        TextEncoding::Utf8 { bom: false },
    )
    ...
    assert!(replay_large_recovery(journal.path()).is_err());
}
