...
pub(super) fn parse_sidecar_stream(
    ...
    file_len: u64,
    path: &Path,
) -> Result<Option<LineIndex>, LargeDocumentError> {
...
pub(super) fn sampled_hash(source: &FileSource, len: u64) -> Result<u32, LargeDocumentError> {
...
fn sidecar_io<T>(path: &Path, result: std::io::Result<T>) -> Result<T, LargeDocumentError> {
    result.map_err(|source| LargeDocumentError::Io {
...
pub(super) fn cleanup_sidecar_cache(
    ...
    let entries = std::fs::read_dir(cache_dir).map_err(|source| LargeDocumentError::Io {
...
impl PageSpool {
    pub(super) fn new() -> Result<Self, LargeDocumentError> {
        let file = tempfile::NamedTempFile::new().map_err(|source| LargeDocumentError::Io {
    ...
    pub(super) fn push(&mut self, page: LinePage) -> Result<(), LargeDocumentError> {
    ...
    pub(super) fn finish(mut self, pages: Vec<LinePage>) -> Result<LinePages, LargeDocumentError> {
        for page in pages {
            self.push(page)?;
        }
        let path = self.file.path().to_path_buf();
...
impl LineIndexBuilder {
    ...
    pub(super) fn finish_line(&mut self, end: u64) -> Result<(), LargeDocumentError> {
    ...
    pub(super) fn finish_file(&mut self) -> Result<(), LargeDocumentError> {
    ...
    pub(super) fn build(self) -> Result<LineIndex, LargeDocumentError> {
