...
impl FileSource {
    pub fn open(path: impl AsRef<Path>) -> Result<Self, LargeDocumentError> {
        let path = path.as_ref().to_path_buf();
        let file = open_shared(&path).map_err(|source| LargeDocumentError::Io {
            path: path.clone(),
            source,
        })?;
        let metadata = file.metadata().map_err(|source| LargeDocumentError::Io {
            path: path.clone(),
            source,
        })?;
        if !metadata.is_file() {
    ...
    pub fn identity(&self) -> Result<FileIdentity, LargeDocumentError> {
        let metadata = std::fs::metadata(&self.path).map_err(|source| LargeDocumentError::Io {
    ...
    pub fn read_range(&self, start: u64, end: u64) -> Result<Vec<u8>, LargeDocumentError> {
        ...
        let len = usize::try_from(end - start).map_err(|_| LargeDocumentError::RangeTooLarge)?;
    ...
    pub fn read_exact_at(
        ...
        while !buffer.is_empty() {
            let read =
                read_at(&self.file, buffer, offset).map_err(|source| LargeDocumentError::Io {
                    path: self.path.clone(),
                    source,
                })?;
            if read == 0 {
    ...
    pub fn sampled_prefix_hash(&self, prefix_len: u64) -> Result<u32, LargeDocumentError> {
    ...
    fn page(
        ...
        page_start: u64,
        file_len: u64,
    ) -> Result<Arc<[u8]>, LargeDocumentError> {
        {
            let mut cache = self
                .cache
                .lock()
...
#[cfg(unix)]
pub(crate) fn sync_parent_directory(parent: &Path) -> Result<(), LargeDocumentError> {
    File::open(parent)
...
#[cfg(not(unix))]
pub(crate) fn sync_parent_directory(_parent: &Path) -> Result<(), LargeDocumentError> {
