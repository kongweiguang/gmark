...
use crate::{
    FileSource, LargeDocumentError, LineIndex, PieceDocument, PreparedUtf8Source, SourceAffinity,
    SourceSelection, TextEncoding, prepare_utf8_source,
};
...
pub enum LargeRecoveryReadStatus {
    Complete,
    TruncatedTail,
}

/// Recovery 与实时 Source 共用同一个 anchor/affinity 真值，不能在落盘时退化为 Range。
pub type LargeRecoverySelection = SourceSelection;

#[derive(Clone, Debug, PartialEq, Eq)]
pub struct LargeRecoveryBase {
    pub path: PathBuf,
    pub len: u64,
...
pub struct RecoveredLargeDocument {
    pub base: LargeRecoveryBase,
    pub journal: LargeRecoveryJournal,
    pub prepared_source: PreparedUtf8Source,
    pub document: PieceDocument,
    pub selection: Option<LargeRecoverySelection>,
    pub view_mode: String,
    pub read_status: LargeRecoveryReadStatus,
}

pub struct LargeRecoveryJournal {
    path: PathBuf,
...
struct PendingReplace {
    ...
    chunk_count: u32,
    chunks: Vec<String>,
    selection: Option<LargeRecoverySelection>,
    view_mode: String,
}
...
impl LargeRecoveryJournal {
    pub fn create(
        ...
        fs::create_dir_all(recovery_dir).map_err(|source| LargeDocumentError::Io {
        ...
        let payload = serde_json::to_vec(&base)
            .map_err(|error| LargeDocumentError::Recovery(error.to_string()))?;
    ...
    pub fn record_replace(
        ...
        let chunk_count = u32::try_from(chunks.len())
            .map_err(|_| LargeDocumentError::Recovery("too many recovery chunks".to_owned()))?;
    ...
    pub fn checkpoint(&self) -> Result<(), LargeDocumentError> {
        match fs::remove_file(&self.path) {
            Ok(()) => Ok(()),
            Err(error) if error.kind() == std::io::ErrorKind::NotFound => Ok(()),
            Err(source) => Err(LargeDocumentError::Io {
    ...
    fn append_command(&self, command: EditRecord) -> Result<(), LargeDocumentError> {
        append_frames(
            &self.path,
...
pub fn replay_large_recovery(
    journal_path: impl AsRef<Path>,
) -> Result<RecoveredLargeDocument, LargeDocumentError> {
    ...
    for command in commands {
        match command {
            LargeRecoveryCommand::Replace { range, chunks } => {
                document.replace_text_chunks(range, chunks.iter().map(String::as_str))?;
            }
            LargeRecoveryCommand::Undo => {
                if !document.undo() {
                    return Err(LargeDocumentError::Recovery(
                        "recovery undo has no matching edit".to_owned(),
                    ));
                }
            }
            LargeRecoveryCommand::Redo => {
                if !document.redo() {
                    return Err(LargeDocumentError::Recovery(
    ...
    Ok(RecoveredLargeDocument {
        base,
        journal: LargeRecoveryJournal {
...
pub fn list_large_recovery_journals(
    recovery_dir: impl AsRef<Path>,
) -> Result<Vec<PathBuf>, LargeDocumentError> {
    let recovery_dir = recovery_dir.as_ref();
    let entries = match fs::read_dir(recovery_dir) {
        Ok(entries) => entries,
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => return Ok(Vec::new()),
        Err(source) => {
            return Err(LargeDocumentError::Io {
                path: recovery_dir.to_path_buf(),
                source,
            });
        }
...
pub fn inspect_large_recovery_base(
    journal_path: impl AsRef<Path>,
) -> Result<LargeRecoveryBase, LargeDocumentError> {
    let journal_path = journal_path.as_ref();
    let file = File::open(journal_path).map_err(|source| LargeDocumentError::Io {
        path: journal_path.to_path_buf(),
        source,
    })?;
...
pub fn large_recovery_has_edits(
    journal_path: impl AsRef<Path>,
...
type JournalRead = (
    LargeRecoveryBase,
    Vec<LargeRecoveryCommand>,
    Option<LargeRecoverySelection>,
    String,
    LargeRecoveryReadStatus,
    u64,
);

fn read_journal(path: &Path) -> Result<JournalRead, LargeDocumentError> {
    let file = File::open(path).map_err(|source| LargeDocumentError::Io {
    ...
    let mut status = LargeRecoveryReadStatus::Complete;
    let mut max_transaction = 0u64;
    loop {
        let (kind, payload) = match read_frame(&mut reader)? {
            FrameRead::End => break,
            FrameRead::Truncated => {
                status = LargeRecoveryReadStatus::TruncatedTail;
                break;
            }
    ...
    let base = base.ok_or_else(|| {
        LargeDocumentError::Recovery("recovery journal has no valid base".to_owned())
    })?;
    Ok((
        base,
        commands,
...
fn consume_edit(
    ...
    commands: &mut Vec<LargeRecoveryCommand>,
    selection: &mut Option<LargeRecoverySelection>,
    ...
    match record {
            ...
            *max_transaction = (*max_transaction).max(transaction);
            ...
            if current.chunks.len() == current.chunk_count as usize {
                ...
                *view_mode = completed.view_mode;
                commands.push(LargeRecoveryCommand::Replace {
        ...
        EditRecord::Undo {
            ...
            *selection = next_selection.map(Into::into);
            *view_mode = next_view_mode;
            commands.push(LargeRecoveryCommand::Undo);
            Ok(true)
        }
        EditRecord::Redo {
            selection: next_selection,
            ...
            *selection = next_selection.map(Into::into);
            *view_mode = next_view_mode;
            commands.push(LargeRecoveryCommand::Redo);
            Ok(true)
        }
    }
}

fn read_frame(reader: &mut BufReader<File>) -> Result<FrameRead, LargeDocumentError> {
    ...
    let Some(decoded) =
        decode_header(&header).map_err(|error| LargeDocumentError::Recovery(error.to_string()))?
    else {
    ...
    if let Err(error) = reader.read_exact(&mut payload) {
        ...
        return Err(LargeDocumentError::Recovery(error.to_string()));
    }
...
fn verify_base(source: &FileSource, base: &LargeRecoveryBase) -> Result<(), LargeDocumentError> {
    let identity = source.identity()?;
...
fn sampled_hash(source: &FileSource, len: u64) -> Result<u32, LargeDocumentError> {
...
fn append_frames(path: &Path, frames: &[Vec<u8>]) -> Result<(), LargeDocumentError> {
    ...
    file.sync_data().map_err(|source| LargeDocumentError::Io {
        path: path.to_path_buf(),
        source,
    })
}

fn atomic_write(path: &Path, bytes: &[u8]) -> Result<(), LargeDocumentError> {
...
impl RecoveryEncoding {
    ...
    fn resolve(self, source: &FileSource) -> Result<TextEncoding, LargeDocumentError> {
        match self {
...
impl From<LargeRecoverySelection> for SelectionRecord {
    fn from(selection: LargeRecoverySelection) -> Self {
...
impl From<SelectionRecord> for LargeRecoverySelection {
