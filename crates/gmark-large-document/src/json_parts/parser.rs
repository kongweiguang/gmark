...
pub(super) fn json_sampled_hash(source: &FileSource, len: u64) -> Result<u32, LargeDocumentError> {
...
pub(super) fn json_cache_data_error(path: &Path, message: String) -> LargeDocumentError {
...
fn parse_json_string(cursor: &mut ByteCursor, end: u64) -> Result<(), LargeDocumentError> {
...
fn parse_hex_quad(cursor: &mut ByteCursor, offset: u64) -> Result<u16, LargeDocumentError> {
    let mut value = 0u16;
    for _ in 0..4 {
...
pub(super) fn object_key_range(
    source: &FileSource,
    range: std::ops::Range<u64>,
) -> Result<std::ops::Range<u64>, LargeDocumentError> {
    let mut cursor = ByteCursor::new(source.clone(), range.start, range.end);
...
pub(super) fn next_non_whitespace(
    cursor: &mut ByteCursor,
) -> Result<Option<(u64, u8)>, LargeDocumentError> {
    while let Some((offset, byte)) = cursor.next_byte()? {
        if !byte.is_ascii_whitespace() {
...
pub(super) fn invalid_json(offset: u64, message: impl Into<String>) -> LargeDocumentError {
    LargeDocumentError::InvalidJson {
        offset,
...
impl ByteCursor {
    ...
    pub(super) fn next_byte(&mut self) -> Result<Option<(u64, u8)>, LargeDocumentError> {
