...
#[test]
fn replaces_one_inline_math_fragment_without_reparsing_other_styles() {
    let mut tree = InlineTextTree::from_markdown("**before** $x^2$ [after](https://example.test)");
    let start = "before ".len();
    let end = start + "$x^2$".len();

    assert!(tree.replace_inline_math_source(start..end, "$\\frac{1}{2}$"));
    assert_eq!(tree.visible_text(), "before $\\frac{1}{2}$ after");
    assert_eq!(
        tree.serialize_markdown(),
        "**before** $\\frac{1}{2}$ [after](https://example.test)"
    );
    assert!(tree.render_cache().style_at(0).bold);
    assert_eq!(
        tree.render_cache().link_at("before $\\frac{1}{2}$ ".len()),
        Some("https://example.test")
    );
    assert_eq!(
        tree.render_cache()
            .inline_math_at("before ".len())
            .map(|math| math.body.as_str()),
        Some("\\frac{1}{2}")
    );
}

#[test]
fn inline_math_replacement_rejects_partial_or_non_math_ranges() {
    let mut tree = InlineTextTree::from_markdown("before $x$ after");
    let start = "before ".len();
    let end = start + "$x$".len();

    assert!(!tree.replace_inline_math_source(start..end - 1, "$y$"));
    assert!(!tree.replace_inline_math_source(start..end, "plain"));
    assert_eq!(tree.serialize_markdown(), "before $x$ after");
}
...
fn parses_reference_style_links_with_generic_normalized_labels() {
    ...
    let definitions =
        super::super::link::parse_link_reference_definitions("[ref links]: https://example.com");
    let tree = InlineTextTree::from_markdown_with_link_references(markdown, &definitions);
...
fn shortcut_reference_detection_does_not_rewrite_reference_images() {
    ...
    let tree = InlineTextTree::from_markdown_with_link_references("![cover][img]", &definitions);
