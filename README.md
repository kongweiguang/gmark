...
gmark 会在打开文件前检查格式、编码和体积。超过约 16 MiB，或行数、结构复杂度较高的文件，会自动进入磁盘支持的大文件模式，避免一次性把完整内容和渲染结果全部放入内存。此时状态栏保持 Source 模式，并把结构化视图、编码重开、日志跟随等低频工具收进更多菜单。

- 支持 Markdown、纯文本、JSON、JSONL/NDJSON、CSV 和 TSV/TAB。
- 大文件源码按可见区域读取，并支持搜索、定位、编辑、撤销与保存。
- JSON 可切换到结构视图，CSV/TSV 可切换到表格视图，Markdown 可查看表格投影。
...
非 Markdown 文本和较大的文件默认使用 Source。JSON、CSV、TSV 等格式还可以从状态栏的更多菜单打开对应结构化视图；这些视图是同一标签页里的文件工具，不会取代磁盘上的原始文本。
